...
let component =
  ...
  in
  let%sub right_sidebar =
    City_result.component ~game ~set_game ~set_error_message
  in
  ...
  in
  let%sub on_restart =
    let%arr set_game = set_game in
    set_game (Game.new_game ())
  in
  let%sub end_screen =
    Endscreen.component ~game ~on_restart ~message:end_message ~set_game
  in
  let%sub info_modal =
